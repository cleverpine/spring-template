...
* This is the Spring Boot template project for all CleverPine microservices, based on API-first driven development.
...
To start with, clone the project: `git clone --recurse-submodules git@github.com:cleverpine/spring-template.git`
...
**Important!** If using 'decodingAuthTokenConfig' bean(default) from `com.cleverpine.template.config.ViravaHelperConfig` only the basic structure of the token is validated. It is expected that the validity and authenticity of the token is checked by an API Gateway
